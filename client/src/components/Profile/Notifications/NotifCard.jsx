...
const NotifCard = ({ props, setNotif, modal = false, openByQ }) => {
  ...
  const { date, title, description, link, notif_type } = props;
  ...
  const unseenStyle = {
    background:
      "linear-gradient(135deg, #151515 35%, #202020 52%, #303030 60%, #151515 65%)",
    backgroundSize: "400%",
  ...
  return (
    <div
      className={`notifcard-container component-fadeIn ${
        deleting && "card-fadein"
      } ${modal && "notifcard-container-modal"}`}
      onClick={handleNotifClick}
    >
      <div
        className={`notifcard-header`}
        style={seen ? { background: "#070707" } : unseenStyle}
      >
        <div className="notifcard-title-container">
          <div className={`notifcard-type ${dotType[notif_type]}`}></div>
          <div
            className={`notifcard-type-border ${
              !seen && correctType[notif_type]
            }`}
          ></div>
          <h2
            style={modal ? { maxWidth: "75%" } : {}}
      ...
      <div className={`notifcard-body ${open && "notifcard-body-open"}`}>
        ...
        <div className="notifcard-buttons">
          {link && !modal && (
            <button className="g-white-button" onClick={() => navigate(link)}>
              ir ...
            </button>
          )}
          {!modal && (

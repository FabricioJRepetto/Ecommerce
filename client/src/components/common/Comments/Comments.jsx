...
const Comments = ({ product_id, comments, allowed }) => {
  ...
  const submitHandler = async () => {
    if (!text) {
      ...
      notification(
        "El texto no puede tener una longitud menor a 5 caracteres",
        "",
        "warning"
      );
    } else if (text.length > 250) {
      notification(
        "El texto no puede tener una longitud mayor a 250 caracteres",
        "",
        "warning"
      );
    } else if (parseInt(calification) === 0) {
      notification(
        "No puedes enviar un comentario sin calificación",
        "",
        "warning"
      );
    } else {
  ...
  return (
    <div className="comments-section-container component-fadeIn">
      <div className="comments-section-header">
        ...
        {
          <div
            className={`comments-section-input-box ${
              allowComment || editMode ? "comment-fadeIn" : "comment-fadeOut"
            }`}
          >
            {!loading ? (
              <>
                <h2>Cuéntanos algo sobre este producto</h2>
                ...
                <div className="comment-button-container">
                  <button className="g-white-button " onClick={submitHandler}>
                    {" "}
                    Publicar{" "}
                  </button>
                  {editMode && (
      ...
      {allComments?.length > 0 ? (
        <div className="comments-container">
          {allComments.map((c) => (
            <CommentCard
              key={c.comment.user_id}
              props={c}
              editable={c.comment.user_id === id}
              edit={editComment}
              editing={editMode}
            />
          ))}
        ...
        <h2 className="comments-no-comments">

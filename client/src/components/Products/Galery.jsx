          ...
          <div className="galery-selector-imgs">
            {imgs.slice(0, 8)?.map((e, index) => (
              ...
              >
                ...
                <img
                  className={`selector-img${!loading ? " visible" : ""}`}
                  src={resizer(e.imgURL, 50)}
                  alt="controller"
                />
                {index > 6 && imgs.length > 8 && !loading && (
                  <p className="galery-text">+{imgs.length - index - 1}</p>
                )}
              </div>
            ))}
          </div>
          <div
            className="galery-selector-pointer"
            style={{ transform: `translateX(${current * 100}%)` }}
          ></div>
        </div>
      </div>
      <div className="preview-container">
        <div onClick={(e) => open(e)} className="ripple-container">
          <span
            className={isRippling ? "ripple" : "content"}
        ...
        {imgs?.map((e, index) => (
          <img
            ...
            onClick={open}
            onLoad={() => {
              index === imgs.length - 1 && setLoading(false);
            }}
            className={`galery-img ${
              current === index && !loading && "visible"
            }`}
            src={e.imgURL}
            alt="img"
          />
